...
@section('content')
    <div class="d-inline-flex justify-content-between w-100 align-items-center mt-4 mb-2">
        <h4 class="text-capitalize">Добавить город</h4>
    </div>
    @if($errors)
        ...
        <div class="row">
            ...
            <div class="form-group col-2">
                ...
                <select id="country_id" name="country_id"
                    ...
                    @foreach($countries as $country)
                        <option
                            value="{{ $country->id }}" {{ $country->id == old('country_id', \App\Models\Country::SLOVAKIA_ID) ? 'selected' : '' }}>{{ $country->name }}</option>
                    @endforeach
                </select>
                @error('country_id')
                <span class="invalid-feedback"><strong>{{ $message }}</strong></span>
            ...
            <div class="form-group col-4">
                ...
                <select id="post_id" name="post_id"
                    ...
                    @foreach($posts as $post)
                        <option
                            value="{{ $post->id }}" {{ $post->id == old('post_id') ? 'selected' : '' }}>{{ $post->title }}</option>
                    @endforeach
                </select>
                @error('post_id')
                <span class="invalid-feedback"><strong>{{ $message }}</strong></span>
            ...
            <div class="col-12">
                <div class="form-group">
                    ...
                    <button type="submit" class="btn btn-success"><span>Добавить город</span></button>
                </div>
            </div>
        </div>
    </form>
