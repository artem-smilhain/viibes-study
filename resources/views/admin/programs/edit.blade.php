...
@section('content')
    <h4 class="mt-4">Редактировать специальность ({{ old('name_sk', $program->name_sk) }}) ⚙️️</h4>
    <h6 class="mb-4">
        <a href="{{ route('admin.programs.index') }}" class="text-capitalize">Специальности</a> /
        <a href="{{ route('admin.programs.show', $program) }}">
            {{ old('name_sk', $program->name_sk) }}
        ...
        <div class="row">
            <div class="input-field col m12">
                <div class="form-group">
                    <label for="name_sk" class="col-form-label text-capitalize">{{ __('app.name_in_slovak') }}</label>
                    <input id="name_sk" name="name_sk" class="form-control @error('name_sk') is-invalid @enderror"
                           value="{{ old('name_sk', $program->name_sk) }}" required>
                    @error('name_sk')
            ...
            <div class="input-field col m12">
                <div class="form-group">
                    <label for="name" class="col-form-label text-capitalize">{{ __('app.name_native') }}</label>
            ...
            <div class="input-field col-6">
                <div class="form-group">
                    <label for="years_of_study" class="col-form-label text-capitalize">{{ __('app.years_of_study') }}</label>
                    <input id="years_of_study" name="years_of_study" class="form-control @error('years_of_study') is-invalid @enderror"
                           value="{{ old('years_of_study', $program->years_of_study) }}" type="number" required>
                ...
                <div class="container p-0">
                    <div class="row p-0">
                        <div class="form-group col-12">
                            ...
                            <textarea class="form-control" id="description" rows="3" name="description" style="min-height: 400px;">
                    ...
                    <div class="row">
                        <div class="form-group col-12">
                            ...
                            <select id="is_in_combination" name="is_in_combination"
                                ...
                                <option value="0" {{ 0 == old('is_in_combination', $program->is_in_combination) ? 'selected' : '' }}>нет</option>
                                <option value="1" {{ 1 == old('is_in_combination', $program->is_in_combination) ? 'selected' : '' }}>да</option>
                            </select>
                        ...
                        <div class="form-group col-12">
                            ...
                            <select id="academic_degree_id" name="academic_degree_id"
                                ...
                                @foreach($academic_degrees as $academic_degree)
                                    <option
                                        value="{{ $academic_degree->id }}" {{ $academic_degree->id == old('academic_degree_id', $program->academic_degree_id) ? 'selected' : '' }}>{{ $academic_degree->degree_title }}</option>
                                @endforeach
                            </select>
                            @error('academic_degree_id')
                            <span class="invalid-feedback"><strong>{{ $message }}</strong></span>
                        ...
                        <div class="form-group col-12">
                            ...
                            <select id="faculty_id" name="faculty_id"
                                ...
                                @foreach($faculties as $faculty)
                                    <option
                                        value="{{ $faculty->id }}" {{ $faculty->id == old('faculty_id', $program->faculty_id) ? 'selected' : '' }}>{{ $faculty->name }}</option>
                                @endforeach
                            </select>
                            @error('faculty_id')
                            <span class="invalid-feedback"><strong>{{ $message }}</strong></span>
                        ...
                        <div class="form-group col-12">
                            ...
                            <select id="direction_id" name="direction_id"
                                ...
                                @foreach($directions as $direction)
                                    <option
                                        value="{{ $direction->id }}" {{ $direction->id == old('direction_id', $program->direction_id) ? 'selected' : '' }}>{{ $direction->name }}</option>
                                @endforeach
                            </select>
                            @error('direction_id')
                            <span class="invalid-feedback"><strong>{{ $message }}</strong></span>
                        ...
                        <div class="form-group col-12">
                            ...
                            <select id="direction_2_id" name="direction_2_id"
                                ...
                                @foreach($directions as $direction)
                                    <option
                                        value="{{ $direction->id }}" {{ $direction->id == old('direction_2_id', $program->direction_2_id) ? 'selected' : '' }}>{{ $direction->name }}</option>
                                @endforeach
                            </select>
